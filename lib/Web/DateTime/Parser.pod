...
Parse the specified argument as an HTML date string [HTML] and return
...
Parse the specified argument as a year string in HTML and return a
L<Web::DateTime> (or C<undef> if failed).  Any error is reported
through the C<onerror> handler.

=item $dt = $parser->parse_month_string ($string)

Parse the specified argument as an HTML month string [HTML] and return
a L<Web::DateTime> (or C<undef> if failed).  Any error is reported
through the C<onerror> handler.
...
Parse the specified argument as an HTML time string [HTML] and return
a L<Web::DateTime> (or C<undef> if failed).  Any error is reported
through the C<onerror> handler.
...
Parse the specified argument as an HTML yearless date string [HTML]
...
Parse the specified argument as an HTML week string [HTML] and return
a L<Web::DateTime> (or C<undef> if failed).  Any error is reported
through the C<onerror> handler.
...
Parse the specified argument as an HTML time-zone offset string [HTML]
