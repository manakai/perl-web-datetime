...
C<xs:dateTimeStamp> value, a valid RFC 3339 C<date-time> value, and a
valid C<http://schema.org/DateTime> value.
...
Serialize the date value represented by the object as an HTML valid
normalized forced-UTC global date and time string [HTML].
...
Serialize the date value represented by the object as an HTML valid
normalized local date and time string [HTML].  Note that time zone is
ignored.
